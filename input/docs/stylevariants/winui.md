...
`MahApps.Styles.ColorPicker.WinUI` is that combo box with a swatch and the name of a colour where the one pick would stand. The fill, the rounded corners, the bottom edge that turns into the accent and the rounded flyout are all that box. What comes down inside the flyout follows it: the groups of swatches are headed by plain text rather than by a filled bar, the swatches are rounded by the same number the box is, and the boxes on the canvas beside them are the WinUI text box and the WinUI up-down. `MahApps.Styles.ColorCanvas.WinUI`, `MahApps.Styles.ColorPalette.WinUI` and `MahApps.Styles.ColorEyeDropper.WinUI` are those three on their own, for a window that shows them without the drop-down. The dropper is the button of this set, down to the line along its bottom edge. [ColorPicker](../controls/ColorPicker) covers it.

`MahApps.Styles.Button.WinUI` is the Windows 10 button in the colours Windows 11 gives it: rounded corners, one unit of border instead of two, and a line along the bottom drawn a shade stronger than the rest of it, which is what makes the button look as if it stood a little above the page. Press it and that line goes. `MahApps.Styles.Button.Accent.WinUI` is the same filled with the accent, framed by a thin white above and a black below. Both carry the minimum height every other control of the set has, so a button holding nothing but an icon is still as tall as the box beside it. [Buttons](../styles/buttons) covers them.
